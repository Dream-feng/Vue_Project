...
- 项目开始日期 2018-08-22，结束日期 2018-08-28
...
4. 使用脚手架命令行工具vue-cli创建vue项目
    ...
    vue init webpack Tarvel  (Tarvel => 项目文件存放位置)
...
2. 首页轮播图
    ...
    >首先在线上创建分支index-swiper，分支都相对于前一个分支，然后同步本地分支
    ```
    git pull
    ```
    >并切换到分支
    ...
    https://blog.csdn.net/mrliber/article/details/78819191 **[配置参考 Swiper官网]**
    ...
    **弱网测试**:F12 > Network > 选择3G网络
    ...
    ```
     .wrapper
        ...
        height:25vw
    ```
     **push代码后**切换回master主干，并合并index-swiper分支代码
    ```
    git checkout master
...
6. 兄弟组件间联动
    ...
    handleTouchMove (e) {
      if (this.touchStatus) {
        ...
        this.timer = setTimeout(() => { // 函数节流，减少 handleTouchMove 执行频率，节约性能
         ...
        }, 16)
      }
    }
...
4. 对全局事件解绑
   ...
   解绑事件：
    ...
    ```
    > 在后面 keep-alive exclue="detail" 时， activated & deactivated 失效
    ...
    <router-link
        ...
        class="abs"
        v-show="showAbs"
        @click.native="removeScrollListener"  // 添加点击事件
    >
...
> 项目打包
  ...
  1. _注：项目目录，会多一个  dist 文件 => 最终要上线的目录给到后端_ 
   ```
  npm run build
   ```
  2. _修改目录_ 
  ...
  build: {
    ...
    assetsPublicPath: '/', // 修改路径
    ...
  }
   ```
> 异步组件的使用<br/>
   _src/router/index.js  详情参考vue.js官网_ 
   ```
  export default new Router({
  routes: [
    {
    ...
    },
    {
      path: '/detail/:id',
      name: 'Detail',
      component: () => import('@/pages/detail/Detail') // 异步组件
    }
  ],
  ...
   ```
